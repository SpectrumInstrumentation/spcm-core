...
qwBufferSize = uint64(lMemsize.value * 2 * lSetChannels.value)  # in bytes. Enough memory for 16384 samples with 2 bytes each, all channels active
...
# wait until acquisition has finished, then calculated min and max
    ...
    if lBitsPerSample.value <= 8:
        ...
        for i in range(0, lMemsize.value, 1):
            for lChannel in range(0, lSetChannels.value, 1):
