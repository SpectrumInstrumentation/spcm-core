...
qwBufferSize = uint64(lMemsize.value * 2 * lSetChannels.value)  # in bytes. Enough memory for 1024 samples with 2 bytes each, all channels active
...
# wait until acquisition has finished, then calculated min and max
    ...
    if dwError != ERR_OK:
        ...
        if lBitsPerSample.value <= 8:
            pData = cast(pvBuffer, ptr8)  # cast to pointer to 8bit integer
        else:
            pData = cast(pvBuffer, ptr16)  # cast to pointer to 16bit integer
        for lSegment in range(0, lMemsize.value // lSegmentSize.value, 1):
            for i in range(0, lSegmentSize.value, 1):
                for lChannel in range(0, lSetChannels.value, 1):
                    ...
                    if pData[lDataPos] < alMin[lChannel]:
                        alMin[lChannel] = pData[lDataPos]
                    if pData[lDataPos] > alMax[lChannel]:
                        alMax[lChannel] = pData[lDataPos]

            # print min and max for all channels in all segments
            sys.stdout.write("Segment {0:d}...\n".format(lSegment))
            sys.stdout.write("\tChannel") 
            for lChannel in range(0, lSetChannels.value, 1):
                sys.stdout.write("\t{0:8d}".format(lChannel))
            sys.stdout.write("\n")
            sys.stdout.write("\tMinimum") 
            for lChannel in range(0, lSetChannels.value, 1):
                sys.stdout.write("\t{0:8d}".format(alMin[lChannel]))
                alMin[lChannel] = 32767
            sys.stdout.write("\n")
            sys.stdout.write("\tMaximum") 
            for lChannel in range(0, lSetChannels.value, 1):
                sys.stdout.write("\t{0:8d}".format(alMax[lChannel]))
