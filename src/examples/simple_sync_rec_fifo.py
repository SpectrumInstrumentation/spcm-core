...
class CardThread (threading.Thread):
    ...
    def run(self):
        ...
        qwToTransfer = uint64(MEGA_B(8))  # card will be stopped after this amount of data has been transferred
        qwTotalMem = uint64(0)           # counts amount of already transferred data
        while qwTotalMem.value < qwToTransfer.value:
            ...
            if dwError != ERR_OK:
                ...
                if lAvailUser.value >= lNotifySize.value:
                    ...
                    pnData = cast(addressof(self.m_pvBuffer.contents) + lPCPos.value, ptr16)  # cast to pointer to 16bit integer
